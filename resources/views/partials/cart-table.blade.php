@if(cart()->cart_lignes()->count())
        ...
        <div class="row p-3 bg-white rounded my-3 align-items-center cart-item shadow-sm m-0" data-id="{{$ligne->id}}">
            ...
            <div class="col-md-2 text-md-start text-center py-md-0 py-2">
                <div class="input-group justify-content-md-end justify-content-center      ">
                    ...
                    <input type="number" style="max-width: 100  px !important;" min="1" max="{{$ligne->article->quantite}}" value="{{$ligne->quantity}}"
                           class="form-control ">
    ...
    <div class="col-md-4 py-2">
        <div class="bg-white p-4 d-flex rounded shadow-sm">
            ...
            <div class="div">
                <p class="text-muted">سعر ثابت: <span class="fw-bold">د.م.</span> <span class="fw-bold">
    {{ isset($selected_city_price) && $selected_city_price ? number_format($selected_city_price, 2, ',', ' ') : number_format(40, 2, ',', ' ') }}
</span></p>
                <p class="text-muted">
                    شحن إلى {{ $cart->city === 'other' ? 'مدينة مغربية أخرى' : $cart->city }}.
                </p>                <a class="change-city trigger cursor-pointer" >تغيير المدينة <i class="fa fa-truck"></i></a>
                <div class="change-city d-none">
                    <select class="form-select" >
                        @foreach($cities as $city)
                        <option @selected($cart->city === $city->nom) value="{{$city->nom}}">{{$city->nom}}</option>
                        @endforeach
{{--                        <option @selected($cart->city === 'tangier') value="tangier">طنجة</option>--}}
    ...
    <div class="col-md-4 py-2">
        <div class="bg-white p-4 d-flex rounded shadow-sm">
            ...
            <h5 class="col-6 m-0 text-end" dir="ltr">{{ number_format(cart()->total + $selected_city_price, 2, ',', ' ') }} <span>  د.م</span>
            </h5>
        </div>
    </div>
